...
public class BazaarFlipper implements Feature {
 ...
 private int claimWaitTicks = 0;
 private static final int MAX_CLAIM_WAIT = 100;
 ...
 case OUTBID -> {
 // --- Claim confirmation: container-agnostic + timeout. ---
 // After cancelling an order the client may return to the book listing screen
 // (not the generic "Bazaar"), which used to strand the mod forever even though
 // the items WERE claimed. Confirm the claim by scanning the actual inventory
 // regardless of which container we landed on, and fall back on a timeout if
 ...
 // landed on the book listing / somewhere unexpected: close and let the
 // state machine re-open the bazaar to continue.
 minecraft.player.closeContainer();
 }
 return;
 }
 return; // items still in transit, keep waiting
 }
 }
 ...
 // Reconciles a task's inInventory / inEnderChest with what is actually on the
 // client right now. Fixes the desync where a click "lags back" (item stays in
 // the container / doesn't arrive in inventory) but the optimistic counters were
 // already updated. The ender chest count is only reconciled while the ender
 // chest (or a backpack) is actually open, since we can't see it otherwise.
 private void reconcileBookCounts(Book book) {
 Task t = task.get(book);
 if (t == null) return;
 int inv = inventoryScanner.findLoreInv(book.getRomanLevel(book.level())).size();
 t.setInInventory(inv);
